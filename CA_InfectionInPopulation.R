...
br = 0.24   #birth rate
rr = 0.08   #recovery rate
dr = 0.24   #death rate
ir = 0.50   #infection rate
mr = 0.25   #mutation rate
vr = 0.21   #vaccination rate

parameters = paste("br=",br,",", "rr=", rr,",", "dr=", dr, ",","ir=", ir,",", "mr=", mr,",", "vr=", vr)
...
pop = matrix(pop_i[range_pop], nrow = (N-2), ncol = (N-2))
colours=c("white", "red", "blue", "orange", "green")
img=image(rbind(pop,-1, c(rep(-1,N-6), 3:0)), axes=FALSE, col=colours)

plot(Time, t_dead[-1], type = "l", ylim = c(0,N^2 /2), col="red", ylab = "No. of people", main= parameters)
